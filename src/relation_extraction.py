...
import numpy as np
import torch
from data_processing.io_utils import save_json, save_text
...
def check_args(args):
    ...
    if args.do_eval and not args.do_train:
        raise RuntimeError(
            "Evaluation mode (do_eval) is only available when do_train is used.\n"
            "You may want to use do_predict instead."
        )

    if args.max_num_checkpoints > 0 and not args.do_eval:
        warnings.warn(
            "Evaluation mode (do_eval) should be set in order to save more than one models."
    ...
    if args.do_eval and args.max_num_checkpoints < 1:
        warnings.warn(
            "You set the eval mode so we expect max_num_checkpoints large than 0 so we set it to 1."
        )
        args.max_num_checkpoints = 1

    if (
        args.do_train
        and Path(args.new_model_dir).exists()
        and not args.overwrite_model_dir
    ):
        raise RuntimeError(
            "{} is exist and overwrite this dir is not permitted.".format(
                args.new_model_dir
            )
        )
...
def app(gargs):
    ...
    if gargs.do_predict:
        ...
        final_prob_results = "\n".join(
            [
                "\t".join(e)
                for e in zip([str(e) for e in preds], [str(e) for e in pred_probs])
            ]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_batch_size", default=8, type=int, help="The batch size for training."
    )
    parser.add_argument(
        "--eval_batch_size", default=8, type=int, help="The batch size for eval."
    )
    parser.add_argument(
        "--learning_rate",
        default=1e-5,
    ...
    parser.add_argument(
        "--log_file", default=None, help="where to save the log information"
    )
    parser.add_argument(
        "--log_lvl", default="i", type=str, help="d=DEBUG; i=INFO; w=WARNING; e=ERROR"
    )
    parser.add_argument(
        "--log_step",
        default=1000,
    ...
    args.logger = TransformerLogger(
        logger_file=args.log_file, logger_level=args.log_lvl
    ).get_logger()
    app(args)
